...
#define SHAKE_IN_ADDR		0x28
#define SHAKE_IN_SIZE		0x2C
#define SHAKE_OUT_ADDR		0x30
#define SHAKE_OUT_SIZE		0x34
#define SHAKE_MODE		0x38
#define SHAKE_CONFIG		0x3C
#define SHAKE_NONCE		0x40
#define SHAKE_ACTION		0x44
#define SHAKE_BUSY		0x48
...
#define ENABLE_BENCH
#ifdef ENABLE_BENCH
unsigned int shake_cycle_monitor = 0;
unsigned int begin, end;

void reset_shake_monitors()
{
  shake_cycle_monitor = 0;
}

#define START_BENCH     (begin = cpu_perf_get(0))

#define END_BENCH       (shake_cycle_monitor = shake_cycle_monitor + (cpu_perf_get(0)  - begin))

unsigned int shakeCycleCount()
{
  return shake_cycle_monitor;
}
#else
void reset_shake_monitors()
{
...
// hardware init
  START_BENCH;
	...
	HWPE_WRITE(0, SHAKE_CONFIG);
	HWPE_WRITE(0x1, SHAKE_ACTION);
	
	while(1){
		status = HWPE_READ(SHAKE_BUSY);
		if (status == 0)
			break;
	}
  END_BENCH;
...
#else
void keccak_absorb( const uint8_t *m,
                                  size_t mlen)
{
  START_BENCH;
  volatile unsigned int status;
	HWPE_WRITE((unsigned int)m, SHAKE_IN_ADDR);
	HWPE_WRITE(mlen/4, SHAKE_IN_SIZE);
	HWPE_WRITE(1, SHAKE_CONFIG);
	HWPE_WRITE(0x1, SHAKE_ACTION);
...
#else
void keccak_finalize()
{
  START_BENCH;
  volatile unsigned int status;
	HWPE_WRITE(3, SHAKE_CONFIG);
	HWPE_WRITE(0x1, SHAKE_ACTION);
	while(1){
		status = HWPE_READ(SHAKE_BUSY);
		if (status == 0)
                                 ...
                                 unsigned int r)
{
  ...
  volatile unsigned int status;
	...
	HWPE_WRITE((r/4)*nblocks, SHAKE_OUT_SIZE);

	HWPE_WRITE((unsigned int)out, SHAKE_OUT_ADDR);
	HWPE_WRITE(4, SHAKE_CONFIG);
	HWPE_WRITE(0x1, SHAKE_ACTION);
...
#else
void keccak_squeeze(uint8_t *out,
                                   size_t outlen)
{
  START_BENCH;
  volatile unsigned int status;
	HWPE_WRITE((unsigned int)out, SHAKE_OUT_ADDR);
	HWPE_WRITE(outlen/4, SHAKE_OUT_SIZE);
	HWPE_WRITE(4, SHAKE_CONFIG);
	HWPE_WRITE(0x1, SHAKE_ACTION);
...
#else
inline void shake128_init()
{
	HWPE_WRITE(0, SHAKE_MODE);
  keccak_init();
}
#endif
...
#else
inline void shake128_absorb(const uint8_t *in, size_t inlen)
{
  keccak_absorb(in, inlen);
}
#endif
...
#else
inline void shake128_squeezeblocks(uint8_t *out, size_t nblocks)
{
...
#else
inline void shake128_squeeze(uint8_t *out, size_t outlen)
{
...
#else
inline void shake256_init()
{
	HWPE_WRITE(1, SHAKE_MODE);
  keccak_init();
}
...
#else
inline void shake256_absorb(const uint8_t *in, size_t inlen)
{
  keccak_absorb(in, inlen);
}
#endif
...
#else
inline void shake256_squeezeblocks(uint8_t *out, size_t nblocks)
{
...
#ifndef HWX
void stream128_nonce(uint16_t nonce, keccak_state *state)
{
  uint8_t t[2];
  t[0] = nonce;
  t[1] = nonce >> 8;
...
void stream256_nonce(uint16_t nonce, keccak_state *state)
{
  uint8_t t[2];
  t[0] = nonce;
  t[1] = nonce >> 8;
...
void mat_poly_gen(uint32_t *a)
{

}
#else
void stream128_nonce(uint16_t nonce)
{
  START_BENCH;
  int status;
	HWPE_WRITE(nonce, SHAKE_NONCE);
	HWPE_WRITE(2, SHAKE_CONFIG);
	HWPE_WRITE(0x1, SHAKE_ACTION);
...
void stream256_nonce(uint16_t nonce)
{
  START_BENCH;
  int status;
	HWPE_WRITE(nonce, SHAKE_NONCE);
...
void mat_poly_gen(uint32_t *a)
{
  START_BENCH;
  int status;
  HWPE_WRITE((unsigned int)a, SHAKE_OUT_ADDR);
  HWPE_WRITE(5, SHAKE_CONFIG);
	HWPE_WRITE(0x1, SHAKE_ACTION);
