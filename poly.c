...
#define ENABLE_BENCH
#ifdef ENABLE_BENCH
static unsigned int pwm_cycle_monitor = 0;
unsigned int begin, end;

void reset_pwm_monitors()
{
  pwm_cycle_monitor = 0;
}
...
#define END_BENCH       (pwm_cycle_monitor = pwm_cycle_monitor + (cpu_perf_get(0)  - begin))

unsigned int pwmCycleCount()
{
  return pwm_cycle_monitor;
}
#else
void reset_pwm_monitors()
{
...
#else
#define HWPE_NTT_DATA  0x0C
#define HWPE_NTT_TRIG	0x18
#define HWPE_NTT_STAT	0x1C

#define HWPE_POLY2_BASE	0x4C
#define HWPE_POLYOUT_BASE	0x50
#define HWPE_NTT_MODE		0x14
...
void poly_pointwise_montgomery(poly *c, const poly *a, const poly *b) {
  ...
  START_BENCH;
  HWPE_WRITE((unsigned int)a, HWPE_NTT_DATA);
	HWPE_WRITE((unsigned int)b, HWPE_POLY2_BASE);
	HWPE_WRITE((unsigned int)c, HWPE_POLYOUT_BASE);
	HWPE_WRITE(0x2, HWPE_NTT_MODE);		// select PWM
	
	HWPE_WRITE(0x01, HWPE_NTT_TRIG);
	
	while(1)
	{
		status = HWPE_READ(HWPE_NTT_STAT);
		if (status == 0)
			break;
	}
  END_BENCH;
                  ...
                  uint16_t nonce)
{
  ...
  //PULP
  uint8_t *buf = (uint8_t*)POLY_BUF_ADDR;
  START_BENCH;
#ifdef HWX
  stream128_init(seed, nonce);
  mat_poly_gen((uint32_t*)a);
#else
